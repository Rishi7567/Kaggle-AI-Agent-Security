...
| `utility_probe.py` | The dimension no one measured: scores a guardrail's **cost**, not just its security — on the security × utility frontier — attacks blocked *and* benign user-workflows preserved. Shows the strict guardrail that best resists attacks also breaks legitimate agent work. |
